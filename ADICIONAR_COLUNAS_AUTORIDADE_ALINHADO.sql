-- Script ALINHADO com a interface - Adicionar colunas para decisões da Autoridade Competente
-- Inclui EXATAMENTE os status que estão no filtro da tela "Processos Administrativos"

-- 1. Adicionar as novas colunas
ALTER TABLE processos_administrativos 
ADD COLUMN IF NOT EXISTS decisao_autoridade VARCHAR(50),
ADD COLUMN IF NOT EXISTS fundamentacao_autoridade TEXT,
ADD COLUMN IF NOT EXISTS data_decisao_autoridade TIMESTAMP WITH TIME ZONE;

-- 2. Atualizar a constraint com TODOS os status da interface
ALTER TABLE processos_administrativos 
DROP CONSTRAINT IF EXISTS processos_administrativos_status_check;

ALTER TABLE processos_administrativos 
ADD CONSTRAINT processos_administrativos_status_check 
CHECK (status IN (
    -- STATUS PADRONIZAÇÃO (do filtro da interface)
    'rascunho',
    'criado_cpm',
    'aprovado_cpm', 
    'assinado_admin',
    'julgamento_ccl',
    'aprovado_ccl',
    'aprovado_juridico',
    'edital_publicado',
    'homologado',
    'finalizado',
    
    -- Status de Rejeição (do filtro da interface)
    'rejeitado_cpm',
    'rejeitado_admin',
    'rejeitado_ccl', 
    'rejeitado_juridico',
    'rejeitado_final',
    
    -- Status Antigos/Legados (do filtro da interface)
    'aguardando_aprovacao',
    'iniciado',
    'analise_cppm',
    
    -- STATUS DESPADRONIZAÇÃO (do filtro da interface)
    'em_criacao_desp',
    'criado_cpm_desp',
    'submetido_autoridade_desp',
    'abertura_autorizada_desp', 
    'aviso_publicado',
    'com_recurso_desp',
    'homologado_desp',
    'excluindo_marcas',
    
    -- Status encontrados no banco atual
    'aguardando_assinatura_orgao',
    
    -- Novos status para fluxo CCL/Autoridade
    'tramitando_ccl',
    'julgado_ccl',
    'ata_julgamento_ccl_homologacao',
    'ata_julgamento_ccl_indeferimento',
    'ata_julgamento_ccl_aprovacao', 
    'ata_julgamento_ccl_rejeicao',
    'diligencia_ccl',
    'homologado_autoridade',
    'indeferido_autoridade',
    'diligencia_autoridade'
));

-- 3. Verificar se as colunas foram criadas corretamente
SELECT column_name, data_type, is_nullable
FROM information_schema.columns 
WHERE table_name = 'processos_administrativos' 
AND column_name IN ('decisao_autoridade', 'fundamentacao_autoridade', 'data_decisao_autoridade')
ORDER BY column_name;

-- 4. Comentários nas colunas para documentação
COMMENT ON COLUMN processos_administrativos.decisao_autoridade IS 'Decisão da Autoridade Competente sobre a recomendação da CCL (homologar, indeferir, diligencia)';
COMMENT ON COLUMN processos_administrativos.fundamentacao_autoridade IS 'Fundamentação jurídica da decisão da Autoridade Competente';
COMMENT ON COLUMN processos_administrativos.data_decisao_autoridade IS 'Data e hora da decisão da Autoridade Competente';

-- 5. Confirmar que todos os registros são válidos
SELECT 'SUCESSO! Banco alinhado com a interface.' as resultado
WHERE NOT EXISTS (
    SELECT 1 FROM processos_administrativos 
    WHERE status NOT IN (
        'rascunho', 'criado_cpm', 'aprovado_cpm', 'assinado_admin', 'julgamento_ccl', 'aprovado_ccl',
        'aprovado_juridico', 'edital_publicado', 'homologado', 'finalizado', 'rejeitado_cpm', 
        'rejeitado_admin', 'rejeitado_ccl', 'rejeitado_juridico', 'rejeitado_final', 
        'aguardando_aprovacao', 'iniciado', 'analise_cppm', 'em_criacao_desp', 'criado_cpm_desp',
        'submetido_autoridade_desp', 'abertura_autorizada_desp', 'aviso_publicado', 'com_recurso_desp',
        'homologado_desp', 'excluindo_marcas', 'aguardando_assinatura_orgao', 'tramitando_ccl',
        'julgado_ccl', 'ata_julgamento_ccl_homologacao', 'ata_julgamento_ccl_indeferimento',
        'ata_julgamento_ccl_aprovacao', 'ata_julgamento_ccl_rejeicao', 'diligencia_ccl',
        'homologado_autoridade', 'indeferido_autoridade', 'diligencia_autoridade'
    )
);