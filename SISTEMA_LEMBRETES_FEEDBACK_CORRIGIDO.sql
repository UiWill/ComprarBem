-- ============================================
-- SISTEMA AUTOMÁTICO DE LEMBRETES PARA FEEDBACK (CORRIGIDO)
...
-- 4. FUNÇÃO PARA VERIFICAR RDMs QUE PRECISAM DE LEMBRETES (CORRIGIDA)
...
BEGIN
    ...
    SELECT 
        ...
        r.material_nome,
        r.material_codigo,
        -- CORRIGIDO: usar data_aprovacao se existir, senão usar atualizado_em para RDMs aprovadas
...
-- 7. VIEW PARA RDMs PENDENTES DE FEEDBACK (CORRIGIDA)
...
DROP POLICY IF EXISTS "rdm_feedbacks_select_policy" ON rdm_feedbacks;
...
DROP POLICY IF EXISTS "rdm_feedbacks_insert_policy" ON rdm_feedbacks;
...
DROP POLICY IF EXISTS "lembretes_feedback_select_policy" ON lembretes_feedback;
CREATE POLICY "lembretes_feedback_select_policy" ON lembretes_feedback
    FOR SELECT USING (tenant_id::text = current_setting('request.jwt.claims', true)::json->>'sub');

DROP POLICY IF EXISTS "lembretes_feedback_insert_policy" ON lembretes_feedback;
...
-- ============================================
-- SIMULAÇÃO DE DADOS PARA TESTE (OPCIONAL)
-- ============================================

-- Descomente para adicionar RDMs de teste com datas antigas para simular lembretes
/*
-- Atualizar algumas RDMs existentes para ter datas antigas (apenas para teste)
UPDATE rdms_online 
SET atualizado_em = NOW() - INTERVAL '20 days'
WHERE status = 'APROVADO' 
AND id = (SELECT id FROM rdms_online WHERE status = 'APROVADO' LIMIT 1);
